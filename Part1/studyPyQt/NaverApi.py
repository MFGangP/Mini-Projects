# NaverApi 클래스 - OpenAPI 인터넷을 통해 데이터를 전달 받음
...
class NaverApi:
    ...
    def get_request_url(self, url):
        req = Request(url)
        # Naver API 개인별 인증
        req.add_header('X-Naver-Client-Id','LlKJbD39wVtTrcTbv9mD')
        req.add_header('X-Naver-Client-Secret','O1Vnd0DC58')
    ...
    def get_post_data(self, post, outputs):
        ...
        # 'Tue, 07 Mar 2023 15:44:00 +0900' 문자열로 들어온 걸 날짜형으로 변경
        pDate = datetime.datetime.strptime(post['pobDate'], '%a, %d %b %Y %H:%M:%S +0900')
        pubDate = pDate.strftime('%Y-%m-%d %H:%M:%S') # 2023-03-07 15:44:00 변경
